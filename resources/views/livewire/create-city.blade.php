<form wire:submit.prevent="store()">
    <div class="row">

        <div class="col-lg-4 mg-t-20 mg-lg-t-0">
            <label for="exampleFormControlInput1" class="tx-17">الولاية</label>
            <select name="state_id" id="state_id" class="form-control  nice-select  custom-select" wire:model='state_id' required>
                <option label="إختر الولايه">
                </option>
                @foreach (App\Models\State::orderBy('name')->get() as $state)
                    <option value="{{ $state->id }}">
                        {{ $state->name }}
                    </option>
                @endforeach
            </select>

            <div>
                <ul>
                    <li>
                        @error('state_id')
                            <span class="text-danger mt-2">{{ $message }}</span>
                        @enderror
                    </li>

                </ul>

            </div>

        </div>
        <div class="col-lg-4 mg-t-20 mg-lg-t-0">
            <label for="exampleFormControlInput1" class="tx-17">المحلية</label>
            <select name="locality_id" id="locality_id" class="form-control  nice-select  custom-select" wire:model='locality_id' required>
                <option label="إختر المحلية">
                </option>
                @foreach (App\Models\Locality::where('state_id',$state_id)->orderBy('name')->get() as $loclaity)
                    <option value="{{ $loclaity->id }}">
                        {{ $loclaity->name }}
                    </option>
                @endforeach
            </select>

            <div>
                <ul>
                    <li>
                        @error('locality_id')
                            <span class="text-danger mt-2">{{ $message }}</span>
                        @enderror
                    </li>

                </ul>

            </div>

        </div>

        <div class="col-lg-4 mg-t-20 mg-lg-t-0">
            <label for="exampleFormControlInput1" class="tx-17"> إسم المدينة</label>
            <input type="text" required class="form-control w-12" id="exampleFormControlInput1" placeholder=""
                wire:model="name">
            <div>
                <ul>
                    <li>
                        @error('name')
                            <span class="text-danger mt-2">{{ $message }}</span>
                        @enderror
                    </li>

                </ul>

            </div>

        </div>

    </div>
    <div class="row">
        <div class="col-lg-4 mg-t-20 mg-lg-t-0">
            <button type="submit"  class="btn btn-success btn-sm">حفظ</button>
        </div>
    </div>
</form>

<script>
   /* document.addEventListener("DOMContentLoaded", () => {
        $('#state_id').select2();
        //$('#state_id').val('5');
        $('#state_id').on('change', function(e) {
            var data = $('#state_id').select2("val");
            @this.set('state_id', data);
            //alert(data);
        });

        window.addEventListener('contentChanged', event => {

            $('#state_id').on('change', function(e) {
                var data = $('#state_id').select2("val");
                @this.set('state_id', data);
            });

        });
    });*/
</script>
