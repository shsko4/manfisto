<div style="direction: rtl; width: auto">
    ...
    <div class="relative">
        <div class="flex items-center justify-between mb-1">
            <div class="flex items-center h-10">
                @if ($this->searchableColumns()->count())
                    <div class="flex rounded-lg w-96 shadow-sm">
                        <div class="relative d-flex focus-within:z-10 mb-4" style="width: 500px">
                            ...
                            <input wire:model.debounce.500ms="search"
                                class="block w-full py-3 pl-10 text-sm border-2 leading-4 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none"
                                placeholder="{{ __('     بحث بإستخدام / ') }} {{ $this->searchableColumns()->map->label->join(', ') }}"
                                type="text" />
                            <div class="absolute inset-y-0 right-0 flex items-center pr-2">
                                <button wire:click="$set('search', null)"
                                    class="text-black hover:text-red-600 focus:outline-none">
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                @endif
                <button class="btn btn-sm btn-success" wire:click="$emit('refreshLivewireDatatable')">تحديث</button>
                ...
                @if ($exportable)
                    <div x-data="{
                        ...
                        <button
                            wire:click="export('{{ auth()->user()->name . ' ' . date('Y-m-d h:i:s') . '' . '.xls' }}')"
                            class="flex items-center px-3 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-400 space-x-2 rounded-md leading-4 hover:bg-green-200 focus:outline-none"><span>{{ __('تصدير') }}</span>
            ...
            <div>
                <div class="table min-w-full align-middle">
                    @unless($this->hideHeader)
                        <div class="table-row divide-x divide-gray-300" style="height: 10px;">
                            @foreach ($this->columns as $index => $column)
                                @if ($hideable === 'inline')
                    ...
                    @endunless
                    <div class="table-row bg-blue-100 divide-x divide-gray-300">
                        @foreach ($this->columns as $index => $column)
                            @if ($column['hidden'])
                                @if ($hideable === 'inline')
                    ...
                    @foreach ($this->results as $row)
                        <div class="table-row p-1 {{ $this->rowClasses($row, $loop) }}">
                            @foreach ($this->columns as $column)
                                ...
                                @else
                                    <div
                                        class="table-cell  @unless($column['wrappable']) whitespace-nowrap truncate @endunless @if ($column['contentAlign'] === 'right') text-right @elseif($column['contentAlign'] === 'center') text-center @else text-left @endif {{ $this->cellClasses($row, $column) }}">
                                        {!! $row->{$column['name']} !!}
                                    </div>
                                @endif
                            @endforeach
        ...
        @unless($this->hidePagination)
            <div class="w-auto bg-white @unless($complex) rounded-b-lg @endunless border-4 border-t-0 border-b-0 @if ($this->activeFilters) border-blue-500 @else border-transparent @endif"
                style="height: 25px;">
                <div class="items-center justify-between sm:flex" style="height: 10px;">
                    {{-- check if there is any data --}}
                    @if (count($this->results))
                        <div class="flex items-center my-2 sm:my-0">
                            <select name="perPage"
                                class="block w-full pl-2 pr-3  text-base border-gray-300 form-select leading-6 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5"
                                wire:model="perPage" style="height: 36px; text-align: center">
                        ...
                        <div class="my-4 sm:my-0" style="position: relative;">
                            <div class="lg:hidden" style="height: 10px;">
                                <span
                        ...
                        <div class="flex justify-end text-gray-600">
                            {{ __('نتيجة') }} {{ $this->results->firstItem() }} - {{ $this->results->lastItem() }}
                            {{ __('من ') }}
                            {{ $this->results->total() }}
                        </div>
                    @endif
