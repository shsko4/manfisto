...
<div class="@if($column['hidden']) hidden @else relative  overflow-hidden align-top flex table-cell @endif" @include('datatables::style-width')>

    @if($column['sortable'])
        <button wire:click="sort('{{ $index }}')"
                class="w-full h-full px-6 py-3 border-b border-gray-200 bg-gray-200 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex justify-between items-center focus:outline-none" style="height: 30px">
            <span class="inline flex-grow @if($column['headerAlign'] === 'right') text-right @elseif($column['headerAlign'] === 'center') text-center @endif  text-black font-extrabold">{{ str_replace('_', ' ', $column['label']) }}</span>
    ...
    @else
        <div class="w-full h-full px-6 py-3 border-b border-gray-200 bg-gray-200 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex justify-between items-center focus:outline-none" style="height: 30px">
            <span class="inline flex-grow @if($column['headerAlign'] === 'right') text-right @elseif($column['headerAlign'] === 'center') text-center @endif text-black font-extrabold" >{{ str_replace('_', ' ', $column['label']) }}</span>
        </div>
    @endif
